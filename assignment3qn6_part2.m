...
colors = ["#00a5cf", "#d1495b", "#3c1642", "#edae49", "#4f772d"];
...
A_body = 0.1;
C_D_body = 0.4;
wing_density_list = [0.1 1 10 74]; %74
v_drone = 10;
...
wingspan_list = linspace(0, 16, n_2);
wing_chord_list = [0.25]; % adapt if needed
beta = linspace(1e-5, pi, n);
alpha_wing = deg2rad(25); %initial angle of wing wrt body
D_body = 0.5 * rho_mars * C_D_body * v_drone^2 * A_body;

required_total_T = zeros(length(wingspan_list), length(wing_density_list));
idrag_wing_list = zeros(length(wingspan_list), length(wing_density_list));
lift_wing_list = zeros(length(wingspan_list), length(wing_density_list));
fdrag_wing_list = zeros(length(wingspan_list), length(wing_density_list));
phi_body = zeros(length(wingspan_list), length(wing_density_list));
weight_list = zeros(length(wingspan_list), length(wing_density_list));
for wing_density_index = 1:length(wing_density_list)
    wing_chord = wing_chord_list(1);
    wing_density = wing_density_list(wing_density_index);
    ...
    for wingspan_index = 1:length(wingspan_list)
        ...
        lift_wing_list(wingspan_index, wing_density_index) = lift_wing;
        idrag_wing_list(wingspan_index, wing_density_index) = idrag_wing;
        fdrag_wing_list(wingspan_index, wing_density_index) = fdrag_wing;
        volume_wing = area_airfoil/1*wing_chord * wingspan;
        mass_wing = wing_density * volume_wing;
        total_mass = total_mass_no_wing + mass_wing;
        mass_list(wingspan_index, wing_density_index) = total_mass;
        weight = total_mass * g_mars;
        weight_list(wingspan_index, wing_density_index) = weight;

        vert_force = weight-lift_wing;
        vert_force_list(wingspan_index, wing_density_index) = vert_force;
        hori_force = idrag_wing+fdrag_wing+D_body;
        hori_force_list(wingspan_index, wing_density_index) = hori_force;
        phi_body(wingspan_index, wing_density_index) = atan(hori_force/vert_force);
        required_total_T(wingspan_index, wing_density_index) = sqrt((hori_force)^2 + (vert_force)^2);
    end
end
...
vi_result = ones(length(wingspan_list), length(wing_density_list));
...
omega_to_move = zeros(wingspan_index,wing_density_index);

for wingspan_index = 1:length(wingspan_list)
    for wing_density_index = 1:length(wing_density_list)
        V_c = V_c_list(wingspan_index, wing_density_index);
        ...
        if TBE(end) ~= 0
            omega_to_move(wingspan_index,wing_density_index) = interp1(TBE, omega_list, required_T_per_rotor(wingspan_index,wing_density_index));
        end
...
P_new_per_rotor = zeros(length(wingspan_list), length(wing_density_list));
for wing_density_index = 1:length(wing_density_list)
    P0 = 1/8 * rho_mars * N_blades * c * Cdo .* omega_to_move(:,wing_density_index).^3 .* R^4;
    P_new_per_rotor(:,wing_density_index) = gamma * P_ideal_per_rotor(:,wing_density_index) + P0;
end
...
hold on
for wing_density_index = 1:length(wing_density_list)
    plot(wingspan_list, required_total_T(:, wing_density_index), 'LineWidth', 2, ...
        'DisplayName', ['Density = ' num2str(wing_density_list(wing_density_index)) ' kg/m$^3$'], 'Color', colors(wing_density_index))
    hold on
end
grid on;
...
figure(7); clf;
for wing_density_index = 1:length(wing_density_list)
    plot(wingspan_list, rad2deg(phi_body(:,wing_density_index)), 'LineWidth', 2, ...
        'DisplayName', ['Density = ' num2str(wing_density_list(wing_density_index)) ' kg/m$^3$'], 'Color', colors(wing_density_index))
    hold on
end
grid on;
...
for wing_density_index = 1:length(wing_density_list)
    plot(wingspan_list, N_rotor*P_new_per_rotor(:,wing_density_index), 'LineWidth', 2, ...
        'DisplayName', ['$\rho _{wing}$ = ' num2str(wing_density_list(wing_density_index)) ' kg/m$^3$'], 'Color', colors(wing_density_index))
    hold on
end
grid on;
set(gca, 'Color', 'white', 'Xcolor', 'k', 'YColor', 'k', 'FontSize', fontsize_general,'GridColor', [0.05, 0.05, 0.05], 'XMinorGrid', 'off', 'YMinorGrid', 'off');
plot(xlim, N_rotor*0.9*[138.1385 138.1385],'black', 'LineStyle', '--', 'LineWidth',2, 'DisplayName', '$P = 0.9 \cdot P_{design}$')
...
figure(9); clf;
for wing_density_index = 1:length(wing_density_list)
    plot(wingspan_list, weight_list(:,wing_density_index), 'LineWidth', 2, ...
        'DisplayName', ['Density = ' num2str(wing_density_list(wing_density_index)) ' kg/m$^3$'], 'Color', colors(wing_density_index))
    hold on
end
grid on
xlabel('Wingspan Length $b$ [m]', 'Interpreter', 'latex', 'FontSize', 22);
ylabel('Weight $W$ [N]', 'Interpreter', 'latex', 'FontSize', 22);
legend('Location', 'best', 'Interpreter', 'latex');

figure(10); clf;
for wing_density_index = 1:length(wing_density_list)
    plot(wingspan_list, mass_list(:,wing_density_index), 'LineWidth', 2, ...
        'DisplayName', ['Density = ' num2str(wing_density_list(wing_density_index)) ' kg/m$^3$'], 'Color', colors(wing_density_index))
    hold on
end
grid on
...
figure(11); clf;
for wing_density_index = 1:length(wing_density_list)
    plot(wingspan_list, omega_to_move(:,wing_density_index), 'LineWidth', 2, ...
        'DisplayName', ['Density = ' num2str(wing_density_list(wing_density_index)) ' kg/m$^3$'], 'Color', colors(wing_density_index))
    hold on
end
grid on
